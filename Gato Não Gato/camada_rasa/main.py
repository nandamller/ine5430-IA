...
from env import *
from utils import exp_decay, load_data
...
num_classes = 2  # gato ou não gato
ytrain = to_categorical(Ytrain, num_classes)
ytest = to_categorical(Ytest, num_classes)

model = tf.keras.Sequential()
model.add(layers.Flatten())
model.add(layers.Dense(HIDDEN_LAYER_NEURONS_NUMBER, kernel_initializer=RANDOM_UNIFORM,
          bias_initializer=RANDOM_UNIFORM, activation=TANH))

model.add(layers.Dense(OUTPUT_LAYER_NEURONS_NUMBER, kernel_initializer=RANDOM_UNIFORM,
          bias_initializer=RANDOM_UNIFORM, activation=SIGMOID))

# Método de treinamento = Gradiente Descendente Estocástico
